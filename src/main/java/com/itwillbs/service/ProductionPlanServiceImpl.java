...
public class ProductionPlanServiceImpl implements ProductionPlanService {
	...
	// 여러 생산 계획 한 번에 등록 처리
	 ...
	 public void registerPlans(List<ProductionPlanDTO> planList) {
	     String today = LocalDate.now().format(DateTimeFormatter.ofPattern("yyyyMMdd")); // 20250723
	     String prefix = "PL-" + today + "-";

	     for (ProductionPlanDTO dto : planList) {
	         // 1. 날짜별 max plan_id 조회
	         String maxId = productionPlanDAO.getMaxPlanIdLike(prefix); // 예: PL-20250723-003
