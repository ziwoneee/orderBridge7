        ...
        <div class="content-wrapper">
          <div class="row">
			...
			<div class="col-12 mb-4">
			  <h3 class="font-weight-bold">출고 관리</h3>
			</div>
		        ...
		        <tbody>
		          <c:forEach var="item" items="${outList}">
		            <tr>
		              ...
		              <td>${item.stockQty}</td>
		              <td>${item.status}</td>
		              <td><fmt:formatDate value="${item.outboundDate}" pattern="yyyy-MM-dd"/></td>
		              <td>
		                <c:choose>
		                  <c:when test="${item.status eq '대기'}">
		                    <button class="btn btn-sm btn-outline-primary" onclick="processOut('${item.outboundId}')">출고처리</button>
		                  </c:when>
		                  <c:otherwise>-</c:otherwise>
		                </c:choose>
		              </td>
		              <td>
		                <button class="btn btn-sm btn-info" onclick="viewDetail('${item.outboundId}')">상세</button>
		              </td>
		            </tr>
		          </c:forEach>
		        </tbody>
